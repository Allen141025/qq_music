...
class Player extends React.Component {
  ...
  componentWillReceiveProps(nextProps) {
    if (nextProps.albumId !== this.props.albumId || (nextProps.albumId!==0 && nextProps.singerPicUrl === '' )) {
      this.props.loadAlbum(nextProps.albumId)
    }
    //fetchCount为并发数量，如果为4，说明四大接口请求都已经完成了，然后再去判断当前这首歌能不能播放
    if(this.state.fetchCount===4){
      if((!nextProps.isCheck.success && nextProps.isCheck.message!=='' && nextProps.songId === this.state.songId) || 
        ...
        <div className="playInfo">
          ...
          <div className="playVideo">
                ...
                <div className="inner" style={{ top: 100 }} ref="inner">
                  {
                    this.props.lyric.map((mc,ind) => (
                      <p time={mc.time} key={ind} className={this.state.currentIndex===ind?'active':''}>{mc.words}</p>
                    ))
                  }
                </div>
              </div>
            ...
            <div className="audioBox">
              <audio loop src={this.props.url} ref="audio" onTimeUpdate={this.process}></audio>
              ...
              <div className="play_acound">
                ...
                <p className={`iconfont muted ${this.state.muted?'icon-jingyin':'icon-yinliang'}`} onClick={this.setMuted}></p>
              ...
              <div className="play_plan_box">
                <div className="play_plan" onClick={this.setProgress}>
                  <div className="play_plan_aa" style={{width:this.state.playPercent+'%'}} ref="playProgress"></div>
                </div>
